...
fig1=figure('Position',[0,0,nx+300+200,ny*3+154+40])

set(fig1)%,'Color','k')
axraw = subplot(11,1,1);
axgl =   subplot(11,1,2);
axw =   subplot(11,1,3);
axwf =  subplot(11,1,4);
...
figxfct=(nx+300)/(nx+300+200);
figyfct=(ny*3+154)/(ny*3+154+40);
figyshift=1-figyfct;
ploty=ny/(ny*3+154);

cE.Position=   [0.0500*figxfct 0.3800*figyfct+figyshift 0.02500*figxfct 0.1500*figyfct];

axraw.Position=[0.1500*figxfct 0.6700*figyfct+figyshift 0.8000*figxfct ploty];
mm5=axraw.Position(3)/(nx*pix2m)*0.005;
axmm.Position =[0.01300*figxfct 0.7400*figyfct+figyshift mm5 0.003*figyfct];
...
axv_l.Position =  [1*figxfct+0.005 axgl.Position(2) 1-figxfct-0.016 axgl.Position(4)];
axGvsl.Position = [1*figxfct+0.005 axw.Position(2)  1-figxfct-0.016 axw.Position(4)];
...
text(scattxl(1),vlim(2)*1.35,{'crack speed: {\it{v}}';'fracture energy: {{\bf{\Gamma}}}'}','fontsize',fontsize*1.1);%,'color','w
...
fname_noloop = [bname,'_strain_images_v2_noloop_white.gif'];
fname = [bname,'_strain_images_v2_white.gif'];
...
for idx=imgidx(1:end-1)%i = 1:(length(imgidx)-7)
    ...
    title(strcat(num2str(round(double(idx)/acquisition_fps,6),'%6.6f s')),...%'frame #  ', string(i)),...
          ...
          'Position',[-0.07,1.0,0],'HorizontalAlignment','right','VerticalAlignment','top')
