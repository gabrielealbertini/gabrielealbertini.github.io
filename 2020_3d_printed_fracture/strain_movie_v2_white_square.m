...
xpad=20
ypad=220
fig1=figure('Position',[0,0,nx+xpad,ny*3+154+ypad])
...
cE.Position=   [0.0500*figxfct -0.2500*figyfct+figyshift 0.02500*figxfct 0.1500*figyfct];

axraw.Position=[0.000*figxfct 0.6700*figyfct+figyshift figxfct ploty];
mm5=axraw.Position(3)/(nx*pix2m)*0.005;
axmm.Position =[0.200*figxfct .9600*figyfct+figyshift mm5 0.003*figyfct];
...
axv_l.Position =  [0.25*figxfct figyshift*0.2 figxfct*0.3, figyshift*0.7];
axGvsl.Position = [0.65*figxfct figyshift*0.2 figxfct*0.3, figyshift*0.7];
...
fname_noloop = [bname,'_strain_images_v2_noloop_white_square.gif'];
fname = [bname,'_strain_images_v2_white_square.gif'];
...
for idx=imgidx(1:end-1)%i = 1:(length(imgidx)-7)
    ...
    title(strcat(num2str(round(double(idx)/acquisition_fps,6),'%6.6f s')),...%'frame #  ', string(i)),...
          ...
          'Position',[0.0,1.1,0],'HorizontalAlignment','left','VerticalAlignment','bottom')
