...
latexify_jmps(plt,fig_width=4,fig_height=2.5)
...
fig = plt.figure()
spec0 = gridspec.GridSpec(ncols=1,nrows=1,left=0.02,right=0.4,top=0.99,bottom=0.6,hspace=0.2)
spec1 = gridspec.GridSpec(ncols=1,nrows=2,left=0.12,right=0.35,top=0.9,bottom=0.25,hspace=0.2)
spec2 = gridspec.GridSpec(ncols=1,nrows=2,left=0.5, right=0.975,top=0.9,bottom=0.25,hspace=0.2)
ax0 = fig.add_subplot(spec0[0, 0])
ax1 = fig.add_subplot(spec1[1, 0])#, sharex=ax)
...
ax1.set_ylabel(r'$\mathrm{applied \ stress}$ $\tau_0/\langle\tau_p\rangle$')
...
ax1.annotate(r'$\mathrm{(Albertini \ et \  al. \ 2020)}$', xy=(-0.6, -0.75), annotation_clip=False)#, xycoords='data'
...
plt.savefig('fig03_movie.png',dpi=300)
plt.savefig('fig03_movie.pdf',dpi=600)
...
ani.save('fig03_movie.gif',writer='imagemagick',dpi=300)
